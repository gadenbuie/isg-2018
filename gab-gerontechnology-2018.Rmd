...
owd <- setwd("~/Dropbox/USF/activityProfiles/analyses/informs2017/")
...
load("~/Dropbox/USF/activityProfiles/data-cache/ch6/dat_ch6_res_core.RData")
dat_ch6_res_core <- ret; rm("ret")
```
...
1. Alert caregivers or system operators
...
ws <- data_frame(
  label = c(rep("w1", 10), rep("w2", 10)),
  x = c(rep("W1", 10), rep("W2", 10)),
  y = c(11:20, 1:10)
)
```

```{r permutation-plots, dependson="permutation-setup"}
permuted_plots <- list()
set.seed(43)
for (i in 1:4) {
  permuted_plots[[i]] <- plot_day_examples(ws, i > 1)
  gg <- cowplot::plot_grid(plotlist = permuted_plots, nrow = 4)
  if (i %in% 2:3) next
  print(gg)
  cat("\n\n---\n\n")
}
...
set.seed(42)
m28_left <- filter_and_jitter(dat_ch6_res_core$House013$moving_28, left_date)
m28_right <- filter_and_jitter(dat_ch6_res_core$House013$moving_28, right_date)
```
...
```{r moving-window-plots, warning = FALSE, include=FALSE}
...
cowplot::plot_grid(
  plot_summarized_with_highlights(dat_ch6_res_core$House013$moving_14, 7, 14, mask_date = TRUE) +
    ggtitle("2 Week Sliding Windows"),
  plot_summarized_with_highlights(dat_ch6_res_core$House013$moving_28, 14, 28, mask_date = TRUE) +
...
- 2016-05-31 -- 2016-06-27
    - June 2016 was an odd month, due to company in home
- 2016-06-19 -- 2016-07-16
    - Back to normalcy
- 2016-08-01 -- 2016-08-28
    - System wide change in data collection
- 2016-10-21 -- 2016-11-17
    - ??
- 2016-12-04 -- 2017-01-02

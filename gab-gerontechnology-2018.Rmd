...
knitr::opts_chunk$set(echo = FALSE, cache = TRUE, results = "asis", fig.height = 7.5, fig.width = 10, fig.retina = 2)
library(tidyverse)
library(ap)
...
# emojdict <- dict
# emojdict["InLivingRoom"]     <- "\U1F6CB"
# emojdict["InKitchen"]        <- "\U1F37D"
# emojdict["InBathroomMaster"] <- "\U1F6C1"
# emojdict["InBedroomMaster"]  <- "\U1F6CF"
# emojdict["!OpenedFrontDoor"] <- "\U2B05 \U1F6AA"
# emojdict["OpenedFrontDoor"]  <- "\U27A1 \U1F6AA"
emojdict <- dict %>% 
  ...
  replace_name("InBathroomMaster", '<i class="fas fa-bath" style="color: #009374"></i> ') %>%
  replace_name("InBedroomMaster", '<i class="fas fa-bed" style="color: #29AFCE"></i> ') %>% 
  replace_name("!OpenedFrontDoor", '<i class="fas fa-sign-out-alt" style="color: #80B0A6"></i>&nbsp;') %>% 
  replace_name("OpenedFrontDoor", '<i class="fas fa-sign-in-alt" style="color: #7396A0"></i>&nbsp;') %>% 
  replace_name("Pause", '<i class="fas fa-pause-circle" style="color: #006484"></i>')
example_actual <- ap:::decode(str_split(example_sequence, "")[[1]], emojdict)
cat('<p class="huge" style="line-height: 1em">', 
    paste0(#'<span class="word">', 
           example_actual[1:10], 
           #"</span>", 
           c(rep("\n--\n", 5), "", "\n--\n", "", "\n--\n"), 
           collapse = rep("&nbsp;", 4)),
...
plot_day_examples <- function(ws, shuffle = FALSE) {
  ...
  ggplot(ws, aes(x, y, fill = label, color = label)) + 
    geom_tile(width = 0.9, height = 0.9, color = "black") + 
    geom_hline(yintercept = 0.375, color = 'black', size = 1.25) +
    coord_equal() + 
    guides(fill = FALSE) + 
    labs(x = "", y = "") + 
    # ylim(-0.75, 10.5) +
    scale_y_continuous(limits = c(-0.75, 10.5), expand = c(0,0))+
...
for (i in 1:4) {
  ...
  gg <- cowplot::plot_grid(plotlist = permuted_plots, ncol = 4)
  print(gg)
  cat("\n\n---\n\n")
}
```
